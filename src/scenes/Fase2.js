...
class Fase2 extends Phaser.Scene {
  ...
  create() {
    ...
    this.add.image(width / 2, height / 2, 'mapa_caverna').setDepth(-1).setDisplaySize(width, height);

    this.add.text(width / 2 - 60, 50, 'Fase 2', {
      fontSize: '20px',
      color: '#ffffff',
    });
    ...
    this.ogros = [
      this.criarOgro(width / 2 + 150, height / 2),
      this.criarOgro(width / 2 - 150, height / 2)
    ];
  ...
  criarOgro(x, y) {
    const vilao = this.physics.add.image(x, y, 'vilao1').setScale(0.15);
    vilao.vida = 25;
